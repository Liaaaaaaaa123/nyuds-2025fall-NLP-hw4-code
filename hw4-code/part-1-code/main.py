...
def create_augmented_dataloader(args, dataset):
    ...
    # 1) 先拿原始的 train split（是原始的、还没tokenize的）
    original_train = dataset["train"]          # 25000 条原始IMDB训练数据

    # 2) 从 train 里随机挑 5000 条做“你刚刚定义的那个 transformation”
    #    注意这里是对原始文本做 custom_transform
    transformed_train_5k = (
        original_train
        .shuffle(seed=42)
        .select(range(5000))
    ...
    # 3) 把 “原始train + 5k变换train” 拼到一起
    #    datasets 有内置的 concatenate_datasets
    from datasets import concatenate_datasets
    augmented_train = concatenate_datasets([original_train, transformed_train_5k])
    # 现在 augmented_train 大约是 25000 + 5000 = 30000 条

    # 4) 对这个新的训练集做跟前面一样的 tokenize
    tokenized_augmented = augmented_train.map(tokenize_function, batched=True, load_from_cache_file=False)

    # 5) 删掉 text，改名成 labels，设成 torch 格式（和上面保持一致）
